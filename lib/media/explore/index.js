...
router.post('/media/explore', bodyParser(), function* () {
  ...
  const callback = this.request.body.callback || null

  // check callback parameters
  if (callback != null) {
    this.assert(callback.id != null && callback.type != null, 400, 'callback must have id and tp attributes')
  ...
  const server = yield balancer.pop()
